...
// remove the blocks that have no forward block in the branch 删除分支中没有前向块的块
// the branch should never use after removing 分支在删除后不应使用
// eg.
// A --> B --> C --> D           (main branch)
//             | --> E --> F     (fork branch)
// if the fork branch call remove(), then the C will not point to E, the E will not point to F,
// 如果分叉分支调用删除（），则 C 不会指向 E，E 不会指向 F，
// but the C still points to D 但 C 仍然指向 D
...
func (b *branch) verifyBlock(cb *cp.Block) error {
	// basically check via block itself 基本上通过块本身检查
	if err := cb.Verify(); err != nil {
		return fmt.Errorf("block struct verify failed:%v", err)
	}

	// deeply check via blockchain context 通过区块链上下文进行深度检查
	// 1. time
...
// verify
// 1. pow
// 2. whether the account has uploaded the evidence before 账户是否上传了之前的证据
